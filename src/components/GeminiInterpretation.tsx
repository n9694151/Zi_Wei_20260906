...
export const GeminiInterpretation: React.FC<GeminiInterpretationProps> = ({
  ...
  const interpContext: InterpretationContext = {
    selectedMajorLimit: activeMajorLimit,
    selectedAnnual: activeAnnual,
    selectedPalaceName,
  };

  const presets = [
    '我適合創業當老闆還是上班發展？適合哪種產業？',
    '我的財運與進財管道如何？會破財或存不住錢嗎？',
    '我的感情婚姻如何？另一半個性如何？如何避免爭執？',
    '當前十年大運與今年流年運勢如何？該衝還是該守？',
    '買房置產與不動產投資運勢如何？何時適合進場？',
    '全盤命格優缺點與人生避坑大白話指南',
  ];
  ...
  const handleOpenGoogleAIWeb = async () => {
    const prompt = generateExpertPrompt(chart, analysis, question, selectedSchool, interpContext);
    ...
    triggerCopyFeedback('已將「紫微命盤結構、三代四化與提問」複製到剪貼簿！已為您在新分頁開啟 Google Gemini，直接貼上（Ctrl+V）即可免費詢問！');
  ...
  return (
    <div className="bg-[#111827] rounded-xl border border-slate-800 shadow-xl p-4 sm:p-6 space-y-5 text-slate-200">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              絕無文言文玄虛套話！嚴密根據本命、大運、流年三盤疊宮與祿權科忌四化，直截了當回答您的問題。
            </p>
          </div>
        </div>

        {/* Current Limit & Annual Context Info */}
        <div className="flex flex-wrap items-center gap-2 text-xs">
          <span className="px-2 py-1 bg-purple-950/80 text-purple-300 border border-purple-800/80 rounded font-mono">
            當前大限：{activeMajorLimit.startAge}~{activeMajorLimit.endAge}歲（{activeMajorLimit.stem}{activeMajorLimit.branch}·{activeMajorLimit.palace}）
          </span>
          <span className="px-2 py-1 bg-cyan-950/80 text-cyan-300 border border-cyan-800/80 rounded font-mono">
            當前流年：{activeAnnual.year}年（{activeAnnual.yearGan}{activeAnnual.yearZhi}·{activeAnnual.annualMingPalace}）
          </span>
        </div>
      </div>
      ...
      {/* Preset Questions */}
      <div>
        <label className="block text-xs font-medium text-slate-400 mb-2">快速大白話快捷提問：</label>
      ...
      <div className="space-y-2">
        <div className="flex gap-2">
          <input
            ...
            placeholder="請直接輸入你想問的事（例如：我適合創業嗎？今年工作能換嗎？感情如何？...）"
          ...
          <button
            ...
            onClick={() => handleAsk()}
            disabled={loading}
            className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 text-xs sm:text-sm font-bold shadow-xs transition-all disabled:opacity-50 shrink-0 cursor-pointer"
          >
            {loading ? (
              <>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 pt-1 text-xs">
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg bg-blue-600/20 hover:bg-blue-600/30 text-blue-300 border border-blue-500/40 font-medium transition-colors cursor-pointer"
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-slate-800/80 hover:bg-slate-700 text-slate-400 hover:text-slate-200 transition-colors cursor-pointer"
          >
          ...
          <div className="flex items-center justify-between border-b border-slate-800 pb-2.5">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="inline-flex items-center gap-1 px-2.5 py-1 rounded bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-mono transition-colors cursor-pointer"
              >
              ...
              <button
                type="button"
                onClick={handleOpenGoogleAIWeb}
                className="inline-flex items-center gap-1 px-2.5 py-1 rounded bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 text-xs font-mono border border-amber-500/40 transition-colors cursor-pointer"
              >
